...
10. **Chapter 10: Domain Model** ← *You are here*
...
# Chapter 10: Domain Model — Blocks, Transactions, and the UTXO Worldview
...
Those bytes are assembled from the header fields (plus a transaction commitment) in `ProofOfWork::prepare_data(...)`, which is why `BlockHeader` contains exactly the fields it does. We cover the proof-of-work loop and the exact hashed byte layout in Chapter 15 (Block Lifecycle and Mining).
...
In the project, the concrete entry point for this flow is `Transaction::new_utxo_transaction(...)` (we read the full implementation in Chapter 14: Transaction Lifecycle).
...
- **Previous**: Chapter 9 (Technical Foundations overview)
- **Next**: Chapter 11 (Blockchain State Management)
