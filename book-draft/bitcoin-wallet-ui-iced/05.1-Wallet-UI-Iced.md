...
Read `main` (Listing 18) as a wiring function with three responsibilities:

- **Initialize async execution**: `init_runtime` creates a Tokio runtime and stores a global handle (Listing 19).
- **Initialize persistence**: the encrypted database is created/opened using a password from `generate_database_password` (Listing 18). The database itself is explained in Chapter 20.
...
The wallet UI’s behavioral logic is concentrated in `update` (Listing 5.6). We read it as an explicit state machine with three recurring patterns:
