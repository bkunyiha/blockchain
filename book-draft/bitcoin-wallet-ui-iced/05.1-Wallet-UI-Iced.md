...
- **Start the MVU loop**: `application(WalletApp::new, update, view).title(title).theme(theme).run()` hands control to Iced.

After `run` is called, the application is driven by messages through `update` and rendering through `view`.
