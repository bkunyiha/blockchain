...
25. <a href="04.1-Desktop-Admin-UI-Iced.md">Chapter 25: Desktop Admin (Iced)</a>
26. **25A: Code Walkthrough** ← *You are here*
...
# Chapter 25A — Desktop Admin Interface (Iced): Boot, Runtime, Types, State, and API Client
...
- **If you’re new to Iced**: read `main.rs` and `runtime.rs` first, then jump to Chapter 25B (`update.rs`) to see how messages trigger async calls.
...
- **If you’re debugging an API call**: start at `api.rs`, then find where the matching `Message::FetchXxx` / `Message::XxxLoaded` is handled in Chapter 25B.
...
## Listing 16A-1.1 — App entrypoint (`bitcoin-desktop-ui-iced/src/main.rs`)
...
- **`application(AdminApp::new, update, view).run()`**: creates initial state once, then enters the event loop:
...
## Listing 16A-2 — Tokio runtime bridge (`bitcoin-desktop-ui-iced/src/runtime.rs`)
...
## Listing 16A-6 — Navigation + events (`bitcoin-desktop-ui-iced/src/types.rs`)
...
- **Request messages** (`FetchInfo`, `FetchBlocks`, `CreateWalletAdmin`, …): these trigger `Task::perform(...)` in Chapter 25B.
...
## Listing 16A-3.1 — Application state (`bitcoin-desktop-ui-iced/src/app.rs`)
...
## Listing 16A-16 — Async API client helpers (`bitcoin-desktop-ui-iced/src/api.rs`)
...
- **Inputs**: `ApiConfig` is built from `AdminApp { base_url, api_key }` in Chapter 25B.
...
- **Why build a client per call?** It keeps the UI side stateless and makes each request self-contained. In Chapter 25B you can read each `Task::perform(spawn_on_tokio(api_call), Message::XxxLoaded)` arm as a direct mapping: *message → API call → loaded message*.

If you want to understand one feature end-to-end, pick one helper here (for example `create_wallet_admin`), then jump to Chapter 25B and find:
...
**[← Chapter 25: Desktop Admin Interface (Iced)](04.1-Desktop-Admin-UI-Iced.md)** | **Chapter 25A** | **[Next: Chapter 25B (Update Loop) →](04.1B-Desktop-Admin-UI-Update-Loop.md)**
