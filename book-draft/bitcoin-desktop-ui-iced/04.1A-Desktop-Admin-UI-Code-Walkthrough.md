...
17. **4.1A: Code Walkthrough** ← *You are here*
...
# Chapter 4.1A — Desktop Admin Interface (Iced): Boot, Runtime, Types, State, and API Client
...
- **If you’re new to Iced**: read `main.rs` and `runtime.rs` first, then jump to Chapter 4.1B (`update.rs`) to see how messages trigger async calls.
...
- **If you’re debugging an API call**: start at `api.rs`, then find where the matching `Message::FetchXxx` / `Message::XxxLoaded` is handled in Chapter 4.1B.
...
- **Request messages** (`FetchInfo`, `FetchBlocks`, `CreateWalletAdmin`, …): these trigger `Task::perform(...)` in Chapter 4.1B.
...
- **Inputs**: `ApiConfig` is built from `AdminApp { base_url, api_key }` in Chapter 4.1B.
...
- **Why build a client per call?** It keeps the UI side stateless and makes each request self-contained. In Chapter 4.1B you can read each `Task::perform(spawn_on_tokio(api_call), Message::XxxLoaded)` arm as a direct mapping: *message → API call → loaded message*.

If you want to understand one feature end-to-end, pick one helper here (for example `create_wallet_admin`), then jump to Chapter 4.1B and find:
...
**📚 [← Chapter 4.1: Desktop Admin Interface (Iced)](04.1-Desktop-Admin-UI-Iced.md)** | **Chapter 4.1A** | **[Next: Chapter 4.1B (Update Loop) →](04.1B-Desktop-Admin-UI-Update-Loop.md)** 📚
