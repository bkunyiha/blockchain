...
1. [Chapter 3: Web API Architecture](../bitcoin-blockchain/web/README.md)
2. **Chapter 4: Desktop Admin Interface** ← *You are here*
...
# Chapter 4.C — Desktop Admin Interface: the View Layer (`view.rs`)

This chapter prints the view layer from `bitcoin-desktop-ui/src/view.rs`.

The key idea: `view(&AdminApp)` renders the whole UI tree, and dispatches into `view_blockchain`, `view_wallet`, `view_transactions`, `view_mining`, `view_health` based on the currently selected menu.

> **Methods involved**
>
> - `view(&AdminApp) -> Element<Message>`
> - `view_blockchain`, `view_wallet`, `view_transactions`, `view_mining`, `view_health`
> - `json_data_display(...)` (shared JSON viewer with copy support)

---

## Diagram: view dispatch + popup hover model

```
view(&AdminApp)
  ├─ renders: top toolbar (base_url/api_key/status)
  ├─ renders: left navigation menu
  │    ├─ simple item: click -> Message::MenuChanged(...)
  │    └─ popup item: mouse hover -> Message::*MenuHovered(true/false)
  │                     click popup entry -> Message::*SectionChanged(...)
  └─ renders: main panel (right side)
       match app.menu {
...
>
> - `view(...)`
> - `Message::*MenuHovered`, `Message::*SectionChanged`
> - `view_*` section renderers

---

## Code Listing 4C-1.1 — `view.rs` (full listing)
...
pub fn view<'a>(app: &'a AdminApp) -> Element<'a, Message> {
    ...
    // Any “actions” (network calls, state changes) are triggered by Messages, handled in `update.rs`.

    // Helper: calculate popup width based on the longest label.
    // This avoids a “jagged” menu where hover popups resize as you move the mouse.
    // Approximation: ~6 px per character + inner padding.
    ...
    // Helper: create blockchain popup items with consistent styling.
    // Each popup entry dispatches `Message::BlockchainSectionChanged(section)`.
...
**📚 [← Chapter 4.B](03B-Desktop-Admin-UI-Update-Loop.md)** | **Chapter 4.C** | **[Next: Chapter 5 (Wallet UI) →](../bitcoin-wallet-ui/04-Wallet-UI.md)** 📚
